...
// проверем есть постоянный хеш участника
...
function submitForm(formObj){
    ...
    setHidden(formObj,'TRAFF_TYPE',get_sbjs.current.typ); //Тип трафика. Возможные значения: utm, organic, referral, typein. Пятого не дано.
    ...
    if(formObj.EMAIL && formObj.EMAIL.value==''){
        formObj.EMAIL.focus();
        alert('Отсутствует обязательное поле E-mail!');
        return false;
    }
    ...
    // Изночально проверяем наличе форм для унифицированного ввода телефона
    if(formObj.PH_COUNTRY != undefined && formObj.PH_CODE != undefined && formObj.PH_NUMBER != undefined){
        ...
        if(formObj.PH_COUNTRY && formObj.PH_COUNTRY.value == ""){
            alert('Введите телефоный код страны');
        ...
        if(!c.test(formObj.PH_COUNTRY.value)){
            formObj.PH_COUNTRY.focus();
            alert('Код страны может быть только числом');
            return false;
        }
        ...
        if(!c.test(formObj.PH_CODE.value)){
            formObj.PH_CODE.focus();
            alert('Код может быть только числом');
            return false;
        }

        if(formObj.PH_NUMBER && formObj.PH_NUMBER.value == ""){
            alert('Введите телефоный номер');
